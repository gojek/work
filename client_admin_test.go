...
// makeKnownJobs registers the named jobs in the known_jobs set so the admin Client methods
// will accept them. The helper does not start any worker.
...
// pause flag set by the admin endpoint. We enqueue a job, pause the queue, then briefly
// run a worker pool and confirm the job is still pending.
...
func TestClient_DeleteAllDeadJobsByJobName(t *testing.T) {
	pool := newTestPool(t)
	ns := "testwork"
	cleanKeyspace(ns, pool)

	insertDeadJob(ns, pool, "alpha", 1, 100)
	insertDeadJob(ns, pool, "alpha", 2, 200)
	insertDeadJob(ns, pool, "beta", 3, 300)
	insertDeadJob(ns, pool, "alpha", 4, 400)

	c := NewClient(ns, pool)

	deleted, err := c.DeleteAllDeadJobsByJobName("alpha")
	assert.NoError(t, err)
	assert.EqualValues(t, 3, deleted)

	jobs, count, err := c.DeadJobs(1)
	assert.NoError(t, err)
	assert.EqualValues(t, 1, count)
	if assert.Equal(t, 1, len(jobs)) {
		assert.Equal(t, "beta", jobs[0].Name)
	}

	// Empty arg is rejected.
	_, err = c.DeleteAllDeadJobsByJobName("")
	assert.Error(t, err)

	// No matches is fine, returns 0.
	deleted, err = c.DeleteAllDeadJobsByJobName("does-not-exist")
	assert.NoError(t, err)
	assert.EqualValues(t, 0, deleted)
}

func TestClient_RetryAllDeadJobsByJobName(t *testing.T) {
	pool := newTestPool(t)
	ns := "testwork"
	cleanKeyspace(ns, pool)
	...
	retried, err := c.RetryAllDeadJobsByJobName("alpha")
	assert.NoError(t, err)
	assert.EqualValues(t, 3, retried)
	...
	llen, err := redis.Int64(conn.Do("LLEN", redisKeyJobs(ns, "alpha")))
	assert.NoError(t, err)
	assert.EqualValues(t, 3, llen)

	_, err = c.RetryAllDeadJobsByJobName("")
	assert.Error(t, err)

	retried, err = c.RetryAllDeadJobsByJobName("does-not-exist")
	assert.NoError(t, err)
	assert.EqualValues(t, 0, retried)
}

func TestClient_DeleteAllDeadJobsByJobName_Paginated(t *testing.T) {
	pool := newTestPool(t)
	ns := "testwork"
	cleanKeyspace(ns, pool)

	for i := range 250 {
		insertDeadJob(ns, pool, "alpha", int64(i), int64(1000+i))
	}
	insertDeadJob(ns, pool, "beta", 999, 9999)
...
func TestClient_RetryAllDeadJobsByJobName_Paginated(t *testing.T) {
	pool := newTestPool(t)
	ns := "testwork"
	cleanKeyspace(ns, pool)
	...
	retried, err := c.RetryAllDeadJobsByJobName("alpha")
	assert.NoError(t, err)
	assert.EqualValues(t, 250, retried)

	_, count, err := c.DeadJobs(1)
	assert.NoError(t, err)
	assert.EqualValues(t, 1, count)
