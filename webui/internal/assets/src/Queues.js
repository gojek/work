...
// Mutation calls. The browser resends HTTP Basic Auth credentials automatically once the
// page itself has authenticated, so we only need credentials: 'include' here.
...
export default function Queues({ url }) {
  ...
  const actionURL = (jobName, action) => `${url}/${encodeURIComponent(jobName)}/${action}`;
  ...
  async function togglePause(queue) {
    ...
    const resp = await mutate(actionURL(queue.job_name, action), { method: 'POST' });
    if (!resp.ok) return;
  ...
  async function saveMaxConcurrency(queue, n) {
    const resp = await mutate(actionURL(queue.job_name, 'max_concurrency'), {
      method: 'PUT',
  ...
  async function purgeQueue(queue) {
    const resp = await mutate(actionURL(queue.job_name, 'purge'), { method: 'POST' });
    if (!resp.ok) return;
    patchRow(queue.job_name, { count: 0 });
  }

  async function resetLock(queue) {
    const resp = await mutate(actionURL(queue.job_name, 'reset_lock'), { method: 'POST' });
    if (!resp.ok) return;
